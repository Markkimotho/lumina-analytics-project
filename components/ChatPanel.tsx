...
import { Send, Bot, User, PlusCircle, Terminal } from 'lucide-react';
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ dataset, charts, onAddChart }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border-l border-slate-800">
      <div className="p-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur text-slate-200 font-semibold flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl p-3.5 ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-slate-800 text-slate-200 rounded-tl-sm'}`}>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900">
        <div className="relative">
          <textarea
            ...
            className="w-full bg-slate-800 text-white rounded-xl pl-4 pr-12 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 resize-none border border-slate-700 placeholder-slate-500"
            rows={1}
